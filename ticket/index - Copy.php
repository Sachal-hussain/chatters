...
<body>
    ...
    <script src="../assets/libs/sweetalert2/sweetalert2.min.js"></script>
      ...
      if(isset($_POST['create_ticket'])){
        ...
        $selected_subscribers = isset($_POST['selected_subscribers']) ? $_POST['selected_subscribers'] : "";
        ...
        if ($sql) {
          $sql_tkt_assign = mysqli_query($connect, "INSERT INTO tkt_assign (tkt_id, description, deadline, priority, reference_tkt, pg_id, departments, shift, type, assigned_by, assigned_to, progress, subscribers, created_at) VALUES ('$tkt_id', '$description', '$deadline', '$priority', '$ticketreference', '$page_id', '$departments', '$shift', '$tickettype','$uid', '$assignedto', '$progress', '$subscribers', '$created_at')") or die(mysqli_error($connect));
          if($sql_tkt_assign){
      ...
      if(mysqli_num_rows($query) > 0){
        while($row=mysqli_fetch_array($query)){
          ...
          $user_details.='
            <tr>
              <td class="dt-select"><input type="checkbox" name="subscribers[]" class="dt-select-checkbox" value="'.$userid.'"></td>
              <td>'.$fullname.'</td>
              <td>'.$department.'</td>
            </tr>'
          ;
        ...
        <div class="modal-body p-4">
          ...
          <form method="post" id="createticketform" class="needs-validation" novalidate enctype="multipart/form-data">
            ...
            <input type="hidden" name="uname" id="uname" value="<?php echo $uname;?>">
             <input type="hidden" name="selected_subscribers" id="selected_subscribers" value="">
  ...
  <script>
    ...
    var table = $('#tbl-subscriber').DataTable({
        ...
        initComplete: function () {
            // Initialize selectedUserIds array with initially selected checkboxes
    ...
    $('#tbl-subscriber_filter input').on('keyup', function () {
        updateSelectedUserIds();
    });

    // Function to update the hidden input field with selected values
    function updateSelectedUserIds() {
        var selectedUserIds = $('.dt-select-checkbox:checked').map(function() {
            return $(this).val();
        }).get();
        // Update the hidden input field with selected values
        $('#selected_subscribers').val(selectedUserIds.join(","));
    }

    // Submit form function
    $('form').submit(function() {
        // Ensure the selectedUserIds are up to date before submitting the form
        updateSelectedUserIds();
    });
});
