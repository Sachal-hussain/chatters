...
$columns = array('cust_id', 'name', 'pgid', 'redeem_limit', 'daily_limit','status', 'addedby', 'created_at', 'updatedby', 'updated_at');
$orderBy = $columns[$orderColumn] ?? 'cust_id';

// Construct the base SQL query
$sql = "SELECT addcustomer.id AS cust_id, name, pgid, redeem_limit,daily_limit, addcustomer.status, addedby, created_at, updatedby, updated_at, pages.pagename 
        FROM addcustomer
        JOIN pages ON pages.id = addcustomer.pgid
        WHERE approval = 0 AND pages.status = 'active'";

// Prepare statement for counting total records without filtering
$stmt = $connect->prepare("SELECT COUNT(*) FROM addcustomer JOIN pages ON pages.id = addcustomer.pgid WHERE approval = 0 AND pages.status = 'active'");
$stmt->execute();
...
if (!empty($search)) {
    $sql .= " AND (LOWER(pagename) LIKE ? 
              OR LOWER(name) LIKE ? 
              OR LOWER(addedby) LIKE ? 
              OR LOWER(redeem_limit) LIKE ? 
              OR LOWER(updatedby) LIKE ? 
...
$filterCountSql = "SELECT COUNT(*) FROM addcustomer JOIN pages ON pages.id = addcustomer.pgid WHERE approval = 0 AND pages.status = 'active'";
