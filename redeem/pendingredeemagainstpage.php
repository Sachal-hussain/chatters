<?php
    ...
    $usertype   = $_SESSION["user_data"]["type"];
    // print_r($uid.'-'.$usertype);
    // exit;
    ...
    // Base query to fetch data with redeem_amount calculation
    $query = "SELECT * FROM redeem WHERE del='0'";

    // Check the user type and apply conditions accordingly
    if ($usertype == 'Manager' || $usertype == 'Webmaster') {
        // No pg_id filter if Manager or Webmaster
        $query .= " AND status='Pending'";
    } else {
        // Apply pg_id filter for other users
        $query .= " AND pg_id IN ($pageIDsString) AND status='Pending'";
    }          
    ...
    $totalRecordsQuery = mysqli_query($connect, "SELECT COUNT(*) as total FROM redeem WHERE status='Pending' AND del='0' ");
