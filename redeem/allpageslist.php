        ...
        <div class="modal-content">
          ...
          <div class="modal-body p-4">
            ...
            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
              ...
              <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-c2c-tab" data-bs-toggle="pill" data-bs-target="#c2c" type="button" role="tab" aria-controls="pills-c2c" aria-selected="false">C2C</button>
              </li>
              <li class="nav-item" role="presentation">
            ...
            <div class="tab-content" id="pills-tabContent">
              <div class="tab-pane fade show active" id="page-cashtag" role="tabpanel" aria-labelledby="pills-home-tab">
                <form method="post" enctype="multipart/form-data" id="redeempaidfrm" action="update_redeem_fields.php">
                  ...
                  <div class="mb-4">
                    <label for="payment_metohd" class="form-label">Payment Method </label>
              ...
              <!-- C2C -->
              <div class="tab-pane fade " id="c2c" role="tabpanel" aria-labelledby="pills-home-tab">
                <form method="post" enctype="multipart/form-data" id="redeempaidfrm_c2c" action="update_redeem_fields.php">
                  <div class="mb-4">
                    <label for="cashtag" class="form-label">Cash Tag</label>
                    <select class="form-control clientcashtag_dropdown" id="c2ccashtag" name="cashtag">

                    </select>
                  </div>
                  <div class="mb-3">
                    <label class="form-label" for="from_cust">From Customer</label>
                    <div class="input-group mb-3 bg-light-subtle rounded-3">
                        
                      <select name="from_cust" id="from_cust" class="form-control form-select customer_list" required>
                  ...
                  <div class="mb-3">
                    <label class="form-label" for="to_cust">To Customer</label>
                    <div class="input-group mb-3 bg-light-subtle rounded-3">
                        
                      <select name="to_cust" id="to_cust" class="form-control form-select customer_list" required>
                 ...
                 <div class="mb-4">
                    ...
                    <select class="form-control" name="action_type" id="action_type" style="display: block !important;">
                      <option value="Pending">Paid</option>
                    </select>
                  </div>
                  <div class="mb-4">
                    <label for="status" class="form-label">Amount</label>
                    <input type="text" class="form-control action-amount" id="amount" name="amount" min="1" step="1" required>
                  </div>
                  <!-- <label for="tkt_content" class="form-label">Content</label>
                  ...
                  <div class="mb-4">
                    <label for="pasteimages" class="form-label">Upload Images </label>
                    <textarea class="form-control demo demo-textarea" placeholder="Paste your images here" style="resize: none;" id="pasteimages" disabled></textarea>
                    <input type="hidden" class="hdn_imagedata" name='hdn_c2cimagedata' id='hdn_c2cimagedata'>
                  ...
                  <input type="hidden" name="redmid" id="redmid" class="redmid">
                  <input type="hidden" name="pgid" id="pgid" class="pgid">
                  <input type="hidden" name="frmaction_type" id="frmaction_type" value="c2c">


                  <div class="modal-footer">
                    <button type="button" class="btn btn-link" data-bs-dismiss="modal">Close</button>
                    <button type="submit" class="btn btn-primary" name="submitc2c" id="submitc2c">Save</button>
                  </div>
              ...
              <div class="tab-pane fade" id="comments-section" role="tabpanel" aria-labelledby="pills-profile-tab">
                <form method="post" enctype="multipart/form-data" id="comments_redeempaidfrm_comment" action="update_redeem_fields.php">
              ...
              <div class="tab-pane fade" id="addedback-section" role="tabpanel" aria-labelledby="pills-profile-tab">
                <form method="post" enctype="multipart/form-data" id="comments_redeempaidfrm_addback" action="update_redeem_fields.php">
                  ...
                  <div class="modal-footer">
                    <button type="button" class="btn btn-link" data-bs-dismiss="modal">Close</button>
                    <button type="submit" class="btn btn-primary" name="submitaddback" id="submitaddback">Save</button>
                  </div>
      ...
      $(document).ready(function() {
        ...
        function handleFormSubmit(formId, modalId) {
          ...
          $(formId).on('submit', function(e) {
            ...
            // submitButton.hide(); // Hide the submit button to prevent multiple submissions

            if (!validateForm($(this))) { // Validate the form based on its ID
              submitButton.show(); // Show the submit button again if validation fails
              return false;
            }
            ...
            $.ajax({
              ...
              success: function(response) {
                  ...
                  if (response.status === 'success') {
                      ...
                      $(formId)[0].reset();
                      $(formId).find('.custom-comment-input').val('');
                      $(formId).find('#comment3').val('');
                      $(formId).find('.hdn_cmntimagedata').val('');
                      $(formId).find('.pasterdmimage').remove();

                      // Fetch and update the table with the new data
              ...
              error: function(xhr, status, error) {
                  var errMsg = xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'An unexpected error occurred.';
                  alert('Error: ' + errMsg); // Show error message
              }
            });
          });
        }
        ...
        function validateForm(form) {
          ...
          var formType = form.find('input[name="frmaction_type"]').val();
          
          if (formType === 'singlecomment') {
            var image = form.find('#hdn_cmntimagedata').val();
            return image !== ''; // Validate if the image field is not empty
          } 
          else if (formType === 'addback') {
            var amount = form.find('#addedback_redeemamount').val();
            return amount !== ''; // Validate if the amount field is not empty
          }
          else if (formType === 'c2c') {
            var image = form.find('#hdn_c2cimagedata').val();
            return image !== ''; // Validate if the amount field is not empty
          }
          return false;
        }

        // Initialize form handlers
        handleFormSubmit('#comments_redeempaidfrm_comment','#paidredeemform');
        handleFormSubmit('#comments_redeempaidfrm_addback','#paidredeemform');
        handleFormSubmit('#redeempaidfrm_c2c','#paidredeemform');
        ...
        function fetchAndUpdateTable(pageid) {
          $.ajax({
            ...
            success: function(data) {
                $('#redeem_details_display').html(data); // Update the table with the new data
            },
            error: function(xhr, status, error) {
                console.error("Error fetching table data:", error); // Log errors for debugging
            }
          });
        }
      });
